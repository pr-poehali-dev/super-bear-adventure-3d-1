"""
Онлайн кооператив: создание/вход в комнаты, синхронизация позиций игроков.
"""
import json
import os
import random
...
def handler(event: dict, context) -> dict:
    ...
    try:
        ...
        if method == "POST" and action == "create":
            ...
            # Cleanup stale rooms for this player
            cur.execute(
                f"DELETE FROM {SCHEMA}.rooms WHERE host_id = %s AND status = 'waiting' AND created_at < NOW() - INTERVAL '10 minutes'",
                (player_id,)
            )
            ...
            code = gen_code()
            cur.execute(
                f"""INSERT INTO {SCHEMA}.rooms (code, host_id, world_id, world_name, world_emoji, status,
                    host_x, host_y, guest_x, guest_y)
                    VALUES (%s,%s,%s,%s,%s,'waiting',0,1.5,2,1.5)""",
        ...
        # ── JOIN ROOM ─────────────────────────────────────────────────────────
            ...
            cur.execute(
                f"UPDATE {SCHEMA}.rooms SET guest_id=%s, status='playing', updated_at=NOW() WHERE id=%s",
                (player_id, rid)
            )
            conn.commit()
        ...
        # ── POLL / SYNC STATE ─────────────────────────────────────────────────
            ...
            cur.execute(
                ...
                f"coins_collected, enemies_alive FROM {SCHEMA}.rooms WHERE code=%s",
                (code,)
            )
            row = cur.fetchone()
            if not row:
            ...
            rid, host_id, guest_id, status, world_id, world_name, world_emoji, \
                hx, hy, h_em, gx, gy, g_em, coins_collected, enemies_alive = row

            role = "host" if player_id == host_id else "guest"

            # Update own position/emotion
            px = body.get("x")
            py = body.get("y")
            emotion = body.get("emotion", "idle")
            coins_col = body.get("coins_collected")
            enemies_al = body.get("enemies_alive")

            updates = ["updated_at=NOW()"]
            params = []
            if role == "host":
                if px is not None:
                    updates.append("host_x=%s"); params.append(float(px))
                if py is not None:
                    updates.append("host_y=%s"); params.append(float(py))
                updates.append("host_emotion=%s"); params.append(emotion[:16])
            else:
                if px is not None:
                    updates.append("guest_x=%s"); params.append(float(px))
                if py is not None:
                    updates.append("guest_y=%s"); params.append(float(py))
                updates.append("guest_emotion=%s"); params.append(emotion[:16])

            if coins_col is not None:
                updates.append("coins_collected=%s"); params.append(str(coins_col))
            if enemies_al is not None:
                updates.append("enemies_alive=%s"); params.append(str(enemies_al))
            ...
            cur.execute(
                ...
                f"status, coins_collected, enemies_alive FROM {SCHEMA}.rooms WHERE id=%s",
                (rid,)
            )
            r2 = cur.fetchone()
            ...
            return ok({
                "role": role,
                "status": st2,
                "has_partner": guest_id is not None,
                "world_id": world_id,
        ...
        # ── GET ROOM INFO ─────────────────────────────────────────────────────
            ...
            cur.execute(
                f"SELECT status, host_id, guest_id, world_name, world_emoji FROM {SCHEMA}.rooms WHERE code=%s",
                (code,)
            )
            row = cur.fetchone()
            if not row:
